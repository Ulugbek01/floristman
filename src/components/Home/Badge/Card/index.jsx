  ...
  return (
    <div className={classes['card']}>
      ...
      <div className={classes['card__img']} onClick={onClick}>
        <img src={item?.images[0] || noImg} alt="card-img" width={278} height={303} />
      </div>
      <p className={classes['card__title']}>{item?.name || 'Title'}</p>
      <div className={classes['card__price-wrapper']}>
        <p className={classes['card__discount-price']}>{item?.price || 'price'} USD</p>
        <p className={classes['card__original-price']}>{item?.discount || 'discount'} USD</p>
      </div>
